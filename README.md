...
图片可以点击选择、拖入上传区或直接用 `Ctrl+V` 粘贴。拍照图建议让整张纸保持平整并露出四个角；选图后点“拍照图：调整四角”，先在原图粗放四个十字准星，再点“按四角裁剪并放大”进入精调。当前准星会同时显示局部像素放大镜。最终四点会做单应性透视还原；它不能修复卷纸、遮挡或近距离广角镜头造成的弯曲畸变。

纸张像素较小的截图会自动高质量放大到分析尺寸，`.cp` 坐标仍按归一化纸张计算；放大能让像素门限正常工作，但无法恢复截图中已经丢失的线条细节。

识别前会先把亮底、暗底和光照不均的图片统一为白底线条证据：暗线、亮线与红蓝相对色差分别提取，再压成中心线供检查。结果页的“线条证据”标签展示这张标准图；几何层会先按八个 22.5° 合法方向聚合模糊短碎片，再执行构造与 cAMV 复核。
